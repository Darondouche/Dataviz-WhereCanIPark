...
async function placeMarkerMap() {
    ...
    data.records.forEach(element => {
        let placeDispo = element.record.fields.grp_disponible;
        let placeTotal = element.record.fields.grp_exploitation;
        let pourcentage = (placeDispo/placeTotal)*100;

        if (pourcentage > 50) {
          let marker = L.marker([element.record.fields.location.lat, element.record.fields.location.lon], {icon: orangeIcon}).addTo(map);
          marker.bindPopup(
            `<a href=” “>${element.record.fields.grp_nom}</a>`
          );
        } else if (pourcentage > 25) {
          let marker = L.marker([element.record.fields.location.lat, element.record.fields.location.lon], {icon: greenIcon}).addTo(map);
          marker.bindPopup(
            `<a href=” “>${element.record.fields.grp_nom}</a>`
          );
        } else {
          let marker = L.marker([element.record.fields.location.lat, element.record.fields.location.lon], {icon: redIcon}).addTo(map);
          marker.bindPopup(
            `<a href=” “>${element.record.fields.grp_nom}</a>`
          );
        }
        console.log(element.record.fields.location);
    });
  }
